...
with the project's own toolchain when the source hash changes (outputs cached on
`/state/build`),
...
outputs from `/state/build`, and still sync runtime-read files such as `config/`. A manifest
...
The DSH adapter's evolution boundary uses two containers. The first performs that dependency
check, build, cache write, and `--version` probe. The second force-materializes the exact
checkpoint, cold-smokes it, and publishes a controller-owned runtime image. Safety only
consumes that already-validated image and starts fresh isolated containers from it; a missing
runtime fails closed instead of rebuilding during measurement. Rebuild this image after
changing `Dockerfile` or `boot.sh`; an older image does not implement this contract.

Attribution: deepseek-harness (github.com/deepseek-ai/deepseek-harness), MIT.

Boot semantics (exact tree): files deleted or renamed by the agent are removed from the baked
tree during exact synchronization; the source hash covers paths as well as contents, so
renames and empty files always re-key the build; the overlay excludes an agent-installed
`node_modules`; and a rebuild first removes every build output and `.tsbuildinfo`, so artifacts
are derived from the current source and a deleted entry point cannot boot from a stale bundle.
An untouched ordinary copy boots via the pristine fast path; runtime-image publication still
force-materializes it before commit.
